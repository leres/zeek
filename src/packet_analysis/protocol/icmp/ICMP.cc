...
bool ICMPAnalyzer::AnalyzePacket(size_t len, const uint8_t* data, Packet* packet)
	...
	ProcessConnectionICMP(id, packet, len);
...
void ICMPAnalyzer::ContinueProcessing(Connection* c, double t, bool is_orig, int remaining, const Packet* pkt)
	...
	if ( ! zeek::detail::ignore_checksums &&
		...
		if ( chksum != 0xffff )
			{
			// TODO: this shouldn't use a hard coded value here
...
void ICMPTransportAnalyzer::AddExtraAnalyzers(Connection* conn)
	{
	// TODO: this was cached in the analyzer manager and probably should be here somehow too
	analyzer::Tag analyzer_connsize = analyzer_mgr->GetComponentTag("CONNSIZE");
...
void ICMPTransportAnalyzer::InitEndpointMatcher(const IP_Hdr* ip_hdr, int len, bool is_orig)
	...
	if ( zeek::detail::rule_matcher )
		...
		if ( ! matcher_state.MatcherInitialized(is_orig) )
			// TODO: this probably shouldn't stay initialized between packets. should it instead be initialized
			// and maintained in the session analyzer?
